...
const [autoDialCurrentItem, setAutoDialCurrentItem] = useState<any>(null);
const [autoDialLoading, setAutoDialLoading] = useState(false);
const [autoDialError, setAutoDialError] = useState("");
  ...
  const connectToOutboundConference = useCallback(
    async ({
      phoneNumber,
      ...
      if (!device || !user?.sipIdentity) {
        throw new Error("Twilio device or SIP identity missing");
      }
  ...
  //         "/voice/outgoing",
  //         { To: phoneNumber, From: `client:${user.sipIdentity}` },
  //         { headers: { Authorization: `Bearer ${token}` } }
  //       );
  
  //       await connectToOutboundConference({
  //         phoneNumber,
  //         conferenceFriendlyName: data.conferenceFriendlyName,
  //       });
  //     } catch (err) {
  //       console.error("startCall failed:", err);
  //       resetCallViewState();
    ...
    async (enabled: boolean) => {
      try {
        ...
        const { data } = await api.patch(
          "/voice/autodial/agent/toggle",
          { enabled },
          { headers: { Authorization: `Bearer ${token}` } }
        );
  
        setAutoDialEnabled(data?.data?.autoDialEnabled ?? enabled);
  
        return data?.data;
      } catch (err: any) {
        console.error("toggleAutoDial failed:", err);
        const msg = err?.response?.data?.message || "Failed to update auto dial mode";
        setAutoDialError(msg);
        throw err;
      }
    },
    [token]
  ...
  const reserveNextAutoDial = useCallback(async () => {
    try {
      setAutoDialLoading(true);
      setAutoDialError("");
  
      const { data } = await api.post(
        "/voice/autodial/agent/next",
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  
      const item = data?.data || null;
      setAutoDialCurrentItem(item);
  
      return item;
    } catch (err: any) {
      console.error("reserveNextAutoDial failed:", err);
      const msg = err?.response?.data?.message || "Failed to reserve next number";
      setAutoDialError(msg);
      return null;
    } finally {
  ...
  const startAutoDialQueueCall = useCallback(
    async (queueItem?: any) => {
      const item = queueItem || autoDialCurrentItem;
  
      if (!item?.id) {
        setAutoDialError("No reserved auto dial number found");
        return;
      }
  
      if (!deviceRef.current || !user?.sipIdentity) {
        setAutoDialError("Twilio device is not ready");
        return;
      }
      ...
      try {
        ...
        const displayName = item.contact
          ? `${item.contact.firstName || ""} ${item.contact.lastName || ""}`.trim()
          : "";
  
        setCustomerNumber(item.phoneNumber);
        setCustomerName(displayName || "Auto Dial");
        setContactInfo(null);
        setDuration(0);
        setCallSid(null);
        setConferenceName(null);
        ...
        const { data } = await api.post(
          `/voice/autodial/agent/queue/${item.id}/start`,
          {},
          { headers: { Authorization: `Bearer ${token}` } }
        );
        await connectToOutboundConference({
          phoneNumber: data.phoneNumber || item.phoneNumber,
          conferenceFriendlyName: data.conferenceFriendlyName,
        });
  
        setAutoDialCurrentItem({
          ...item,
          status: "dialing",
        });
      } catch (err: any) {
        console.error("startAutoDialQueueCall failed:", err);
        const msg = err?.response?.data?.message || "Failed to start auto dial call";
        setAutoDialError(msg);
        resetCallViewState();
        safeSetStatus("READY");
      } finally {
  ...
  const clearAutoDialCurrentItem = useCallback(() => {
    setAutoDialCurrentItem(null);
    setAutoDialError("");
  }, []);
  ...
  if (disabled) {
    ...
    autoDialCurrentItem: null,
    autoDialLoading: false,
    autoDialError: "",
    toggleAutoDial: async (_enabled: boolean) => {},
    reserveNextAutoDial: async () => null,
    startAutoDialQueueCall: async (_item?: any) => {},
