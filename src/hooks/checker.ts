...
export function useTwilioDevice({ disabled = false }: { disabled?: boolean } = {}) {
  const { user, token,forceLogoutLocal } = useAuth();
  ...
  const incrementMessage = useCallback(() => {
    // ✅ Guard: electronAPI exist kare tou hi notify karo
    window?.electronAPI?.notify?.("New Message", "You have a new message");
  ...
  const getAgentsList = useCallback(async () => {
    try {
      ...
      const filtered = (agents.data || []).filter((item: any) => item.phoneNumber !== null);
  ...
  const handleForceLogout = useCallback(
    async (data?: any) => {
      try {
        ...
        if (window?.electronAPI?.closeCallWindow) {
        ...
        console.error("force_logout cleanup failed:", err);
        window.location.href = "/#/login";
      }
    },
    [forceLogoutLocal]
  ...
  useEffect(() => {
    ...
    /**
     * Backend agar hyphen event use kar raha hai:
     */
    messageSocket.emit("join-user-room", { userId: user.id });
  
    /**
     * Backend agar underscore event use kar raha hai:
     * Dono emit karne se issue nahi hoga.
     */
    messageSocket.emit("join_user_room", user.id);
  
    messageSocket.on("force_logout", handleForceLogout);
    messageSocket.on("force-logout", handleForceLogout);
  
    return () => {
      messageSocket.off("force_logout", handleForceLogout);
      messageSocket.off("force-logout", handleForceLogout);
  
      messageSocket.emit("leave-user-room", { userId: user.id });
      messageSocket.emit("leave_user_room", user.id);
  
      messageSocket.disconnect();
      messageSocketRef.current = null;
    };
  }, [disabled, token, user?.id, handleForceLogout]);
  ...
  useEffect(() => {
    ...
    const onCallEnded = async (data: any) => {
      console.log(data)
      if (
        data?.conferenceFriendlyName &&
        conferenceName &&
        data.conferenceFriendlyName !== conferenceName
      ) {
        return;
      }
...
useEffect(() => {
  ...
  // ✅ YE LISTENERS YAHAN MOVE KARO — socket guaranteed exist karta hai
  const onNewMessage = () => {
    incrementMessage();
  };

  const onNewVoicemail = ({ number }: { number?: string }) => {
...
}, [disabled, token, user?.id, handleForceLogout, incrementMessage]);
    ...
    const init = async () => {
      try {
        ...
        device.on("registered", async () => {
          ...
          try {
            const { data: activeCall } = await api.get("/voice/agent/active-call", {
              headers: { Authorization: `Bearer ${token}` },
            });
        
            if (!activeCall?.hasActiveCall || !activeCall?.conferenceName) return;
        
            console.log("🔄 Active call found, rejoining via device.connect():", activeCall);
        
            const device = deviceRef.current;
            if (!device) return;
        
            // ✅ Approach A — direct device.connect() with rejoin params
            // outboundTwiml backend pe rejoin check karega
            // Koi incoming ring nahi, koi incoming view nahi
            const conn = await device.connect({
              params: {
                To: activeCall.customerNumber || "rejoin",
                identity: user.sipIdentity,
                conference: activeCall.conferenceName,
                rejoin: "true", // ✅ outboundTwiml mein ye check hoga
              },
            });
        
            activeCallRef.current = conn;
        
            // ✅ State immediately set karo
            safeSetStatus("ON_CALL");
            setConferenceName(activeCall.conferenceName);
            setCustomerName(activeCall.customerName || "Unknown Caller");
            setCustomerNumber(activeCall.customerNumber || "");
            setIsMuted(false);
            setIsOnHold(false);
        
            // ✅ Timer resume from elapsed
            clearTimer();
            let elapsed = activeCall.durationSeconds || 0;
            setDuration(elapsed);
            timerRef.current = window.setInterval(() => {
              elapsed += 1;
              setDuration(elapsed);
            ...
            // ✅ Accept pe callSid set karo — hangup ke liye zaroori
            conn.on("accept", () => {
              const newSid = (conn.parameters as any)?.CallSid;
              if (newSid) {
                setCallSid(newSid);
                console.log("🔄 Rejoin accepted, callSid:", newSid);
              }
            });
        
            window?.electronAPI?.notify?.(
              "📞 Call Reconnected",
              `Rejoined: ${activeCall.customerName || activeCall.customerNumber}`
            );
            ...
            console.log("No active call to rejoin:", err?.message);
          }
        });
  ...
  }, [token, user?.id, user?.name, resetCallViewState, safeSetStatus, updateAgentStatus]);
  ...
  const startCall = useCallback(
    ...
    [token, user?.sipIdentity, resetCallViewState, safeSetStatus, updateAgentStatus]
  );

  // ─────────────────────────────────────────────────────────
  // startAutoDialCall — AutoDial ke liye special method
  //
  // Normal startCall se FARQ:
  //   startCall      → /voice/outgoing CALL karta hai (conference set)
  //                    phir device.connect()
  //
  //   startAutoDialCall → /voice/outgoing NAHI bulata
  //                    backend ne startReservedAutoDialCall mein
  //                    pehle se prepareOutboundCall chala diya
  //                    Sirf device.connect() karta hai us
  //                    conference mein jo backend ne banayi
  // ─────────────────────────────────────────────────────────
  const startAutoDialCall = useCallback(
    async (phoneNumber: string, conferenceFriendlyName: string) => {
      const device = deviceRef.current;
      if (!device || !user?.sipIdentity) {
        console.error("startAutoDialCall: device ya sipIdentity missing");
        return;
      }
      ...
      try {
        ...
        // ✅ Directly device connect karo — /voice/outgoing nahi
        const conn = await device.connect({
          params: {
            To: phoneNumber,
            identity: user.sipIdentity,
            conference: conferenceFriendlyName, // backend ne jo banaya
          },
        });
        ...
        conn.on("error", async (err) => {
          console.error("AutoDial call error:", err);
          activeCallRef.current = null;
          resetCallViewState();
          safeSetStatus("READY");
          await updateAgentStatus("AVAILABLE");
        ...
        console.log(`✅ AutoDial device connected → ${conferenceFriendlyName}`);
      } catch (err) {
        console.error("startAutoDialCall failed:", err);
        resetCallViewState();
        safeSetStatus("READY");
      }
    },
    [user?.sipIdentity, resetCallViewState, safeSetStatus, updateAgentStatus]
  );
  ...
  }, [conferenceName, callSid, token, resetCallViewState, safeSetStatus, updateAgentStatus]);
  ...
  if (disabled) {
    return {
      ...
      startAutoDialCall:      async (_phoneNumber: string, _conf: string) => {},
      hangup:                 async () => {},
      toggleMute:          () => {},
      toggleHold:          async () => {},
      resetMissed:         () => {},
