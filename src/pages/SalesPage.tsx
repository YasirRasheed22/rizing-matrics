...
export default function SalesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get(`/voice/sales?${params.toString()}`, {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const res = await api.get(`/voice/leads/stats?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setStats({
        ...
        avgSale: totalSales > 0 ? Math.round(revenue / totalSales) : 0,
      });
    } catch (err) {
  ...
  useEffect(() => { fetchData(); }, [token, page, limit, dateFilter, customStart, customEnd]);
  useEffect(() => { fetchStats(); }, [token, dateFilter, customStart, customEnd]);

  const dateLabel = dateFilter === "custom" && customStart && customEnd
    ? `${customStart} → ${customEnd}` : DATE_LABELS[dateFilter];

  const statCards = [
    { label: "Total Sales", value: stats.totalSales, icon: BadgeCheck, bg: isDark ? "rgba(22,163,74,0.12)" : "#F0FDF4", tc: "#15803D" },
    { label: "Revenue", value: stats.revenue, icon: DollarSign, bg: isDark ? "rgba(124,58,237,0.12)" : "#F5F3FF", tc: accentMain },
    { label: "Avg. Sale", value: stats.avgSale, icon: CreditCard, bg: isDark ? "rgba(124,124,240,0.12)" : "#EFF6FF", tc: accentMain },
    { label: "Total Calls", value: stats.totalCalls, icon: Phone, bg: isDark ? "rgba(234,88,12,0.12)" : "#FFF7ED", tc: "#C2410C" },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: bgPage, fontFamily: "'Inter', -apple-system, sans-serif", color: textPrimary }}>
      ...
      <div style={{ background: cardBg, borderBottom: `1px solid ${headerBorder}`, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: textPrimary, margin: 0, letterSpacing: "-0.02em" }}>Sales</h1>
          <p style={{ color: textMuted, fontSize: 13, margin: "2px 0 0" }}>{sales.length} records · {dateLabel}</p>
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <Link to="/leads/add-lead" style={{ textDecoration: "none" }}>
            <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", background: "#16A34A", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13, color: "#fff", fontWeight: 600 }}>
          ...
          <div ref={datePickerRef} style={{ position: "relative" }}>
            ...
            <AnimatePresence>
              {showDatePicker && (
                <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.97 }} style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 12, boxShadow: isDark ? "0 10px 30px rgba(0,0,0,0.6)" : "0 8px 24px rgba(0,0,0,0.10)", zIndex: 100, padding: 8, minWidth: 190 }}>
                  {["all", "today", "week", "month"].map((f) => (
                    <button key={f} onClick={() => { setDateFilter(f); setShowDatePicker(false); setCustomStart(""); setCustomEnd(""); }} style={{ display: "block", width: "100%", textAlign: "left", padding: "8px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: dateFilter === f ? 600 : 400, background: dateFilter === f ? "rgba(124,124,240,0.15)" : "transparent", color: dateFilter === f ? accentMain : textSecondary }}>
                      {DATE_LABELS[f]}
                    </button>
                  ))}

                  <button onClick={() => setDateFilter("custom")} style={{ display: "block", width: "100%", textAlign: "left", padding: "8px 12px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: dateFilter === "custom" ? 600 : 400, background: dateFilter === "custom" ? "rgba(124,124,240,0.15)" : "transparent", color: dateFilter === "custom" ? accentMain : textSecondary }}>
                    Custom Range
                  </button>
                  ...
                  {dateFilter === "custom" && (
                    <div style={{ padding: "10px 8px 4px", borderTop: `1px solid ${headerBorder}`, marginTop: 4 }}>
                      ...
                      <button onClick={() => { if (customStart && customEnd) setShowDatePicker(false); }} disabled={!customStart || !customEnd} style={{ width: "100%", padding: "7px", borderRadius: 7, border: "none", background: customStart && customEnd ? accentMain : "#E5E7EB", color: customStart && customEnd ? "#fff" : "#9CA3AF", fontSize: 12, fontWeight: 600, cursor: customStart && customEnd ? "pointer" : "not-allowed" }}>
                        Apply Range
                      </button>
                    </div>
                  )}
      ...
      <div style={{ padding: "24px 28px" }}>
        ...
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: `1px solid ${headerBorder}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: textSecondary }}>All Sales</span>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: isDark ? "rgba(255,255,255,0.03)" : "#F9FAFB" }}>
                  {["Client", "Phone", "Services", "Amount", "Payment", "Created", ""].map((h) => (
              ...
              <tbody>
                {isFetching || isLoading ? (
                  <tr>
                    <td colSpan={7} style={{ padding: "80px 0", textAlign: "center", color: textMuted }}>Loading sales...</td>
                  </tr>
                ) : sales.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: "80px 0", textAlign: "center", color: textMuted }}>
                      <User size={32} style={{ color: "#D0D5DD", margin: "0 auto 10px", display: "block" }} />
                      <p style={{ fontSize: 14, fontWeight: 600, color: textPrimary, margin: "0 0 4px" }}>No sales found</p>
                      <p style={{ fontSize: 13, color: textMuted }}>Try adjusting your date filter</p>
                    </td>
                  </tr>
                ) : (
                  sales.map((item, idx) => {
                    const av = avatarColor(item.clientName || "");
                    const serviceText = Array.isArray(item.services) && item.services.length > 0 ? item.services.join(", ") : "—";
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "14px 20px", fontSize: 13, color: textPrimary, maxWidth: 260 }}>
                          <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} title={serviceText}>
                            {serviceText}
                          </div>
                        </td>

                        <td style={{ padding: "14px 20px", whiteSpace: "nowrap" }}>
                          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 10px", borderRadius: 20, background: isDark ? "rgba(22,163,74,0.15)" : "#ECFDF3", color: isDark ? "#4ADE80" : "#067647", fontSize: 12, fontWeight: 700 }}>
                        ...
                        <td style={{ padding: "14px 20px", whiteSpace: "nowrap", fontSize: 13, color: textMuted }}>
                          {item.paymentType ? (
                            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 10px", borderRadius: 20, background: item.paymentType === "recurring" ? (isDark ? "rgba(124,124,240,0.15)" : "#EFF6FF") : (isDark ? "rgba(255,255,255,0.08)" : "#F9FAFB"), color: item.paymentType === "recurring" ? accentMain : textMuted, fontSize: 12, fontWeight: 600, textTransform: "capitalize" }}>
                              <CreditCard size={12} />
                              {item.paymentType}
                            </span>
          ...
          {totalPages > 0 && (
            <div style={{ padding: "12px 20px", borderTop: `1px solid ${headerBorder}`, background: isDark ? "rgba(255,255,255,0.03)" : "#F9FAFB", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <select value={limit} onChange={(e) => setLimit(Number(e.target.value))} style={{ padding: "5px 8px", borderRadius: 8, border: `1px solid ${cardBorder}`, fontSize: 13, color: textSecondary, background: cardBg, outline: "none" }}>
