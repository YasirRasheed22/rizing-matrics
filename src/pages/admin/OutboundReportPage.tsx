// src/pages/admin/OutboundReportPage.tsx
// @ts-nocheck
import React, { useEffect, useRef, useState } from "react";
import {
  Filter, ChevronDown, ChevronLeft, ChevronRight,
  Download, ArrowLeft, Phone, PhoneCall, PhoneOff,
...
export default function OutboundReportPage() {
  ...
  const [stats, setStats] = useState({ totalOutbound: 0, connected: 0, notConnected: 0, talkTime: 0 });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get(`/voice/reports/outbound?${p}`, { headers: { Authorization: `Bearer ${token}` } });
      setOutboundData(res.data.data || []); setStats(res.data.stats);
      setTotalPages(res.data.pagination?.totalPages || 1);
      setTotalRecords(res.data.pagination?.total || 0);
    } catch { setError("Failed to load outbound calls. Please try again."); }
  ...
  const triggerExport = (type: "excel" | "pdf") => {
    ...
    // const apiBase   = (window as any).__API_URL__ || import.meta.env?.VITE_API_URL || "/api";
    const url       = `${API_URL}/voice/reports/outbound/export?${buildExportParams()}`;

    xhr.open("GET", url);
    if (token_val) xhr.setRequestHeader("Authorization", `Bearer ${token_val}`);

    // Fake incremental progress since most servers don't send Content-Length
    let fakeVal = 0;
    ...
    xhr.onload = () => {
      ...
      try {
        ...
        if (type === "excel") {
          ...
          XLSX.utils.book_append_sheet(wb, ws, "Outbound Calls");
          setExportProgress(95);
          XLSX.writeFile(wb, `Outbound_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
        } else {
          const doc = new jsPDF();
          doc.setFontSize(16);
          doc.text("Outbound Calls Report", 105, 20, { align: "center" });
          ...
          setExportProgress(95);
          doc.save(`Outbound_Report_${new Date().toISOString().slice(0, 10)}.pdf`);
        }
    ...
    return (
      <div ref={ref} style={{ position: "relative" }}>
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => setOpen(o => !o)}
          ...
          <Clock size={14} color={isActive ? accentMain : textMuted} />
          {label}
          {isActive ? (
            <span onClick={handleClear} style={{ width: 16, height: 16, borderRadius: "50%", background: isDark ? "rgba(124,124,240,0.25)" : "rgba(91,91,214,0.20)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
              <X size={9} color={accentMain} />
            </span>
          ) : (
            ...
            <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.97 }} transition={{ duration: 0.15 }}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 14 }}>
                <div style={{ width: 26, height: 26, borderRadius: 7, background: accentBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Clock size={13} color={accentMain} />
                </div>
    ...
    <div style={{ minHeight: "100vh", background: pageBg, paddingBottom: 48, fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: textPrimary, letterSpacing: "-0.3px" }}>Outbound Report</h1>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 24 }}>
        <StatCard label="Total Outbound" value={stats.totalOutbound} icon={Phone}     accent={accentMain} />
      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: `1px solid ${thBorder}`, background: toolbarBg, flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: textPrimary }}>Outbound Calls</span>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <motion.button
              ...
              disabled={!outboundData.length || exportStatus !== "idle"}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 9, border: "1px solid rgba(16,185,129,0.3)", background: exportType === "excel" && exportStatus !== "idle" ? "rgba(16,185,129,0.15)" : "rgba(16,185,129,0.08)", color: "#059669", fontWeight: 700, fontSize: 12.5, cursor: (!outboundData.length || exportStatus !== "idle") ? "not-allowed" : "pointer", opacity: (!outboundData.length || (exportStatus !== "idle" && exportType !== "excel")) ? 0.45 : 1, transition: "all 0.15s", fontFamily: "inherit" }}>
            ...
            <motion.button
              ...
              disabled={!outboundData.length || exportStatus !== "idle"}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 9, border: "1px solid rgba(239,68,68,0.3)", background: exportType === "pdf" && exportStatus !== "idle" ? "rgba(239,68,68,0.15)" : "rgba(239,68,68,0.08)", color: "#DC2626", fontWeight: 700, fontSize: 12.5, cursor: (!outboundData.length || exportStatus !== "idle") ? "not-allowed" : "pointer", opacity: (!outboundData.length || (exportStatus !== "idle" && exportType !== "pdf")) ? 0.45 : 1, transition: "all 0.15s", fontFamily: "inherit" }}>
              <FileText size={13} />
            ...
            <tbody>
              {isLoading || isFetching ? (
                <tr><td colSpan={7} style={{ textAlign: "center", padding: "56px 0" }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: textMuted }}>Loading outbound calls…</p>
                </td></tr>
              ) : error ? (
                ...
                <tr><td colSpan={7} style={{ textAlign: "center", padding: "56px 0" }}>
                  ...
                  <p style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 700, color: textPrimary }}>{isDurationActive ? "No calls match this duration range" : "No outbound calls found"}</p>
                ...
                outboundData.map((item, idx) => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.015 }}
                    ...
                    <td style={tdStyle}>{item.agentName || item.from}</td>
                    <td style={tdStyle}>{item.contactName || item.to}</td>
          ...
          <motion.div
            ...
            <motion.div
              ...
              <motion.button
                ...
                onClick={exportStatus === "done" ? () => { setExportStatus("idle"); setExportType(null); setExportProgress(0); } : cancelExport}
